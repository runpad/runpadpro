...
begin
 if h_api1=nil then
  h_api1:=HackCreateProcessA(true);
 if h_api2=nil then
...
try
  ...
  try
//      if IsWord then
//        WHost.CommandBars.Item['Menu Bar'].Enabled:=false
//      else
//        EHost.CommandBars.Item['WorkSheet Menu bar'].Enabled:=false;

      // если надо скрывать конкретные пункты меню-----------------------
          ...
          begin
            ...
            begin
              if (varMenuItem.Controls.Item[i].ID = dFileHistoryID) or
                 ...
                 (varMenuItem.Controls.Item[i].ID = dViewTaskPane) then
              begin
                try
                  if varMenuItem.Controls.Item[i].Visible then varMenuItem.Controls.Item[i].Enabled:=false;
                except
      ...
      //Запрещаем гиперссылку в контекстном меню
 ...
 try
  ...
  try
     //Открываем меню
      ...
      // Если надо открыть все пункты меню--------------------
      if IsWord then
       varMainMenu:=WHost.CommandBars.Item['Menu Bar']
      else
       varMainMenu:=EHost.CommandBars.Item['WorkSheet Menu bar'];
      ...
      if IsWord then
      begin
        //WHost.KeyBindings.ClearAll;  - this is the main problem in office2000
      end;
      // Для Excel не нужно
      //else
      //begin
      ...
      //Открываем гиперссылку в контекстном меню
