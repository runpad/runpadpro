...
var sIFR=function()
{
	...
	var f=function()
	{
		...
		if(!f.o&&!f.m&&al.createElementNS)try{al.createElementNS(a,"i").innerHTML=""}catch(e){f.e=true}f.p=f.c||(f.a&&f.k<312);
		return f
	}();
	function at()
	{
	...
	function l(e)
	{
		if(c==false||k.bIsDisabled==true||((f.e&&f.m||f.l)&&e==null&&b==false)||(al.body==null||al.getElementsByTagName("body").length==0)) return false; return true
	}
	function m(n)
	{
	...
	function o(p,q,r,s,t)
	{
		...
		for(var i=0,h=U.length;i<h;i++)
		{
			...
			ac=o(p,aa,r);
Z="txt="+m(ac.u).replace(/\+/g,"%2B").replace(/&/g,"%26").replace(/\"/g, "%22").normalize() + S + "&w=" + V + "&h=" + W + ac.t;F(p,"sIFR-replaced");
			if(ad==null||!f.g)
			{
				if(!f.g)
				{
				...
				else
				{
					if(f.d)
					{
						...
						C(ab,"quality","best");
						C(ab,"wmode",T);
						C(ab,"bgcolor",N)} else {ab=A("embed");
		...
		if(am.attachEvent)am.attachEvent("onload",af); 
			...
			if(!f.c&&(al.addEventListener||am.addEventListener))
			{
				if(f.a&&f.k>=132&&am.addEventListener) 		
					am.addEventListener("load",function(){setTimeout("sIFR({})",1)},false);
				else{
